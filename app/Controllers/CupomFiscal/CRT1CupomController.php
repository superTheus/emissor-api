...
/**
 * Controller para emissão de NFC-e para empresas do Simples Nacional (CRT 1)
 *
 * Para empresas optantes pelo regime do Simples Nacional.
 * Utiliza ICMSSN com CSOSN (Código de Situação da Operação no Simples Nacional)
 */
class CRT1CupomController extends BaseCupomFiscalController
{
  /**
   * Processa os impostos específicos do Simples Nacional para NFC-e
   */
  ...
  protected function generateIcmssnData($produto, $item)
  {
    ...
    $std->CSOSN = $this->resolveCSOSN($produto);
  ...
  /**
   * Resolve o CSOSN baseado na empresa ou produto
   */
  protected function resolveCSOSN($produto)
  {
    $validCsosn = ['101','102','103','300','400','201','202','203','500','900'];

    // Primeiro tenta pegar do produto
    if (isset($produto['csosn'])) {
      $csosn = (string) $produto['csosn'];
      if (in_array($csosn, $validCsosn, true)) {
        return $csosn;
      }
    }

    // Depois tenta da empresa
    $csosn = (string) ($this->company->getSituacao_tributaria() ?? '');
    if (!empty($csosn) && in_array($csosn, $validCsosn, true)) {
      return $csosn;
    }

    // Fallback seguro para NFC-e
    return '102'; // Tributada sem permissão de crédito (mais comum para NFC-e)
  }
