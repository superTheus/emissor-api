...
class CRT1Controller extends BaseFiscalController
{
  ...
  protected function generateIcmssnData($produto, $item)
  {
    ...
    $cson = isset($produto['cst_icms']) ? $produto['cst_icms'] : '102';
    $cson = UtilsController::validaCSOSN($cson) ? $cson : '102';
    $std->CSOSN = $cson;
    ...
    switch ($std->CSOSN) {
      ...
      case '900': // Outros
        ...
        $this->totalImpostoProduto += floatval($std->vICMS);
        // ST se houver
  ...
  protected function generateIBSCBSData(array $produto, int $item)
  {
    ...
    $this->totalIBS += floatval($std->gIBSUF_vIBSUF) + floatval($std->gIBSMun_vIBSMun);
