...
class NotaServicoController extends Connection
{
    ...
    protected function buildDados(string $municipio, string $cnpj, int $serie, int $numero): array
    {
        ...
        $idDps = $this->nfse->gerarIdDPS($municipio, $cnpj, $serie, $numero);
    ...
    protected function buildRegTrib(array $fiscal): array
    {
        ...
        if (isset($fiscal['opSimpNac']))   $opSimpNac   = intval($fiscal['opSimpNac']);
        if (isset($fiscal['regApTribSN'])) $regApTribSN = intval($fiscal['regApTribSN']);
        if (isset($fiscal['regEspTrib']))  $regEspTrib  = intval($fiscal['regEspTrib']);
