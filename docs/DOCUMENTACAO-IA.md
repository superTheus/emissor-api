...
2. Não suponha autenticação, autorização, rate limit, validação central, migrations, testes ou OpenAPI: não existem neste snapshot.
...
| 11 | POST | `/fiscal/nfe/cancel` | `FiscalController::cancelNfe` | `CancelRequest` | `FiscalDocumentResponse` |
| 12 | POST | `/fiscal/nfe/carta` | `FiscalController::gerarCC` | `CCeRequest` | `CCeResponse` |
...
| 15 | POST | `/fiscal/nfce/cancel` | `CupomFiscalController::cancelNfce` | `CancelRequest` | status/message |
...
| 20 | POST | `/fiscal/certicate/test` | `EmissoesController::verifyCertificate` | `CertificateTest` | empresa/cnpj |
| 21 | GET | `/fiscal/certicate/test/{cnpj}` | `UtilsController::testCertificate` | path CNPJ | dados certificado |
...
CancelRequest:
  ...
  NFe_min_justificativa: 15
  NFCe_prevalidation: absent
CCeRequest:
  cnpj: validado
  chave: acesso direto sem validação
  carta: acesso direto sem validação
CertificateTest:
  certificado: base64 ou data URI
  senha: string
EmissaoFilter:
...
Cancelamento NF-e: valida campos → carrega emissão local/protocolo → `sefazCancela` → aceita 128/135 → protocola evento → DANFE com marca de cancelado → PDF → insere evento `CANCELAMENTO`. Não marca a linha `emissoes` como cancelada.

CC-e: sequência local +1 → `sefazCCe` → aceita 128/135/136 → atualiza `sequencia_cc` → DACCE → PDF → evento `CC`. Se render falhar, retorna 200 sem arquivo/evento, mas sequência já foi incrementada.
...
Cancelamento NFC-e só chama SEFAZ e retorna mensagem; não salva evento/XML/PDF.
...
common_fiscal_status:
  ...
  403: retorno SEFAZ não autorizado/não tratado
...
verifyCertificate:
  200: {empresa, cnpj}
  400: base64/senha/PFX/parse/validade inválidos
testCertificate_saved:
  200: {emissao, dt_vencimento, nome, documento}
  404: empresa ausente
  500: pode imprimir linhas de erro OpenSSL
```
...
Fachada de CRT. Resolve a empresa/CRT no construtor e resolve novamente para cancelamento/CC-e via `handlerFromRequestData`.
...
|---|---|---|---|
| SEC-001 | crítica | entrypoint/router/company | sem auth + CORS `*` + segredos expostos |
...
| API-002 | média | CC-e/cancel NFC-e | validação de campos ausente |
...
| `makeHandlerByCrt` | constrói CRT1/2/3/4 ou encerra com 400 |
| `handlerFromRequestData` | repete validação/resolução para eventos |
| `createNfe`, `cancelNfe`, `gerarCC` | delegação ao handler |
